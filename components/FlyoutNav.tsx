    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${scrolled
          ? "bg-pink-50/90 backdrop-blur-md border-b border-pink-200 py-4 shadow-sm shadow-pink-100"
          : "bg-transparent py-6"
          }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          {/* LOGO */}
          <Link href="/" className="flex items-center gap-2 font-bold text-xl text-neutral-800 z-50">
            <Image
          ...
          <div className="hidden lg:flex items-center gap-8 font-semibold text-lg text-neutral-600">
            <FlyoutLink href="/our-care-services">
              Our Care Services
            </FlyoutLink>
            <FlyoutLink href="/med-spa">
              Med Spa
            </FlyoutLink>
...
const MobileMenu = ({ onClose }: { onClose: () => void }) => {
  return (
    <motion.div
      ...
      className="fixed inset-0 top-0 z-40 bg-white pt-24 px-6 pb-6 flex flex-col h-screen overflow-y-auto lg:hidden"
    >
      {/* Mobile Links - All Simple Links Now (No Dropdowns) */}
      <div className="flex flex-col space-y-6">
        <Link href="/our-care-services" className="text-lg font-semibold text-neutral-600 border-b border-pink-50 pb-4">
          Our Care Services
        </Link>
        <Link href="/med-spa" className="text-lg font-semibold text-neutral-600 border-b border-pink-50 pb-4">
          Med Spa
        </Link>
        <Link href="/about" className="text-lg font-semibold text-neutral-600 border-b border-pink-50 pb-4">
          About
        </Link>
        <Link href="/contact" className="text-lg font-semibold text-neutral-600 border-b border-pink-50 pb-4">
          Contact
        </Link>
      </div>
      ...
      <div className="mt-auto pt-8 border-t border-pink-100 flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-xs font-bold uppercase tracking-wider text-neutral-400">Our Location</span>
            <span className="text-base font-bold text-neutral-600">South Barrington</span>
          </div>
        </div>
...
const FlyoutLink = ({
  ...
  return (
    ...
    >
      <Link
        href={href}
        className={`relative transition-colors inline-block py-4 ${open ? 'text-pink-600 font-semibold' : 'text-neutral-600'}`}
      >
        {children}
        <span
